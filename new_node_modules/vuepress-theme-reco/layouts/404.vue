...
export default defineComponent({
  setup (props, ctx) {
    ...
    const proxy = instance && instance.proxy ? instance.proxy : (ctx.root || props)

    const noFoundPageByTencent = computed(() => {
      return proxy.$themeConfig && proxy.$themeConfig.noFoundPageByTencent !== false
    })
    ...
    onMounted(() => {
      if (noFoundPageByTencent.value && proxy.$site && proxy.$site.base) {
        const dom = document.createElement('script')
        dom.setAttribute('homePageName', '回到首页')
        dom.setAttribute('homePageUrl', proxy.$site.base)
